  ...
  return (
    ...
    >
      <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4">
        <div className="flex flex-wrap gap-2 md:gap-4 overflow-x-auto pb-2 md:pb-0 no-scrollbar">
          {(["overall", "rewards", "contributions"] as FilterType[]).map(
            (filter) => (
              <button
                key={filter}
                onClick={() => onFilterChange(filter)}
                className={`flex-shrink-0 px-4 md:px-5 py-2 md:py-2.5 rounded-[12px] font-semibold text-[13px] md:text-[14px] transition-all duration-300 hover:scale-105 whitespace-nowrap ${
                  activeFilter === filter
        ...
        <div className="relative z-[100] w-full md:w-auto">
          <button
            ...
            className="w-full md:w-auto flex items-center justify-between md:justify-start gap-2 px-4 py-2.5 rounded-[12px] backdrop-blur-[30px] bg-white/[0.15] border border-white/25 hover:bg-white/[0.2] hover:scale-105 transition-all duration-300"
          >
            <span
          ...
          {showDropdown && (
            <div className="absolute right-0 mt-2 w-full md:w-[200px] backdrop-blur-[40px] bg-white/[0.18] border-2 border-white/30 rounded-[12px] shadow-[0_8px_32px_rgba(0,0,0,0.15)] overflow-hidden z-[100] animate-dropdown-in">
              {loading ? (
