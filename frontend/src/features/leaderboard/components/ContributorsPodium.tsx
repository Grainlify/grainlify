  ...
  if (isMobile) {
    return (
      <div className="flex flex-col items-center gap-4 mt-6">
        {/* 1st Place */}
        <div className={`flex flex-col items-center transition-all duration-700 delay-600 w-full ${
          isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'
        }`}>
          <div className="relative backdrop-blur-[30px] bg-gradient-to-br from-[#c9983a]/30 to-[#d4af37]/20 border-2 border-[#c9983a]/60 rounded-[20px] p-6 w-full max-w-[280px] shadow-[0_8px_32px_rgba(201,152,58,0.35)] hover:shadow-[0_12px_40px_rgba(201,152,58,0.5)] transition-all duration-300 group">
            <div className="absolute inset-0 bg-gradient-to-br from-[#c9983a]/10 to-transparent rounded-[20px] animate-pulse-glow" />
            <div className="relative">
              <div className="relative w-16 h-16 rounded-full bg-gradient-to-br from-[#c9983a] to-[#a67c2e] flex items-center justify-center mx-auto mb-3 border-2 border-[#d4af37] shadow-xl text-2xl overflow-hidden">
                {topThree[0].avatar.startsWith('http') ? (
                  <img src={topThree[0].avatar} alt={topThree[0].username} className="w-full h-full object-cover" />
                ) : (
                  topThree[0].avatar
                )}
                <Crown className="absolute -top-5 left-1/2 -translate-x-1/2 w-5 h-5 text-[#d4af37] animate-float" />
              </div>
              <div className="text-center">
                <div className={`text-[14px] font-bold mb-1 transition-colors ${
                  theme === 'dark' ? 'text-[#f5f5f5]' : 'text-[#2d2820]'
                }`}>{topThree[0].username}</div>
                <div className="text-[22px] font-black text-[#c9983a] animate-number-glow">{topThree[0].score}</div>
          ...
          <div className="flex items-center justify-center gap-1.5 backdrop-blur-[20px] bg-gradient-to-br from-[#c9983a]/40 to-[#d4af37]/30 border-2 border-[#c9983a]/70 rounded-[12px] px-4 py-2 shadow-md animate-slide-up min-h-[44px]">
            <Trophy className="w-5 h-5 text-[#c9983a] animate-bounce-gentle" />
        ...
        {/* 2nd Place */}
        {showSecond && (
          <div className={`flex flex-col items-center transition-all duration-700 delay-700 w-full ${
            isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'
          }`}>
            <div className="backdrop-blur-[30px] bg-gradient-to-br from-white/[0.25] to-white/[0.15] border-2 border-white/40 rounded-[18px] p-5 w-full max-w-[260px] shadow-[0_6px_24px_rgba(0,0,0,0.1)] hover:shadow-[0_8px_28px_rgba(0,0,0,0.15)] transition-all duration-300 group">
              <div className="relative">
                <div className="w-14 h-14 rounded-full bg-gradient-to-br from-[#c9983a]/80 to-[#a67c2e]/70 flex items-center justify-center mx-auto mb-3 border-2 border-white/30 shadow-lg text-xl group-hover:rotate-12 transition-transform duration-300 overflow-hidden">
              ...
              <div className="text-center">
                ...
                <div className="text-[18px] font-black text-[#c9983a]">{topThree[1].score}</div>
                <div className={`text-[11px] transition-colors ${
                  theme === 'dark' ? 'text-[#d4d4d4]' : 'text-[#7a6b5a]'
                }`}>pts</div>
              </div>
            </div>
            <div className="flex items-center justify-center gap-1.5 backdrop-blur-[20px] bg-white/[0.2] border border-white/30 rounded-[10px] px-3 py-1.5 shadow-sm animate-slide-up-delayed min-h-[44px]">
              <Medal className="w-4 h-4 text-[#a89780]" />
        ...
        {/* 3rd Place */}
        {showThird && (
          <div className={`flex flex-col items-center transition-all duration-700 delay-800 w-full ${
            isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'
          }`}>
            <div className="backdrop-blur-[30px] bg-gradient-to-br from-white/[0.25] to-white/[0.15] border-2 border-white/40 rounded-[18px] p-5 w-full max-w-[260px] shadow-[0_6px_24px_rgba(0,0,0,0.1)] hover:shadow-[0_8px_28px_rgba(0,0,0,0.15)] transition-all duration-300 group">
              <div className="relative">
                <div className="w-14 h-14 rounded-full bg-gradient-to-br from-[#b89968]/80 to-[#9a7d4f]/70 flex items-center justify-center mx-auto mb-3 border-2 border-white/30 shadow-lg text-xl group-hover:rotate-12 transition-transform duration-300 overflow-hidden">
              ...
              <div className="text-center">
                ...
                <div className="text-[18px] font-black text-[#c9983a]">{topThree[2].score}</div>
                <div className={`text-[11px] transition-colors ${
                  theme === 'dark' ? 'text-[#d4d4d4]' : 'text-[#7a6b5a]'
                }`}>pts</div>
              </div>
            </div>
            <div className="flex items-center justify-center gap-1.5 backdrop-blur-[20px] bg-white/[0.2] border border-white/30 rounded-[10px] px-3 py-1.5 shadow-sm animate-slide-up-more-delayed min-h-[44px]">
              <Medal className="w-4 h-4 text-[#b89968]" />
              <span className={`text-[15px] font-bold transition-colors ${
                theme === 'dark' ? 'text-[#f5f5f5]' : 'text-[#2d2820]'
              }`}>#3</span>
            </div>
